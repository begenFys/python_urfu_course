...
class TestStatistics(unittest.TestCase):
    ...
    def testExtractYears(self):
        self.assertListEqual(
            t.extract_years(self._stat), list(map(str, range(2004, 2013)))
        )

    def testExtractGeneral_CorrectOrder(self):
        self.assertHasOrder(t.extract_general(self._stat), func=self._cmp)
    ...
    def testExtractMale_CorrectOrder(self):
        self.assertHasOrder(t.extract_general_male(self._stat), func=self._cmp)
    ...
    def testExtractFemale_CorrectOrder(self):
        self.assertHasOrder(t.extract_general_female(self._stat), func=self._cmp)
    ...
    def testExtractYear_CorrectOrder(self):
        for year in t.extract_years(self._stat):
            self.assertHasOrder(t.extract_year(self._stat, year), func=self._cmp)
    ...
    def testExtractYearMale_CorrectOrder(self):
        for year in t.extract_years(self._stat):
            self.assertHasOrder(t.extract_year_male(self._stat, year), func=self._cmp)
    ...
    def testExtractYearFemale_CorrectOrder(self):
        for year in t.extract_years(self._stat):
            self.assertHasOrder(t.extract_year_female(self._stat, year), func=self._cmp)
    ...
    def testCorrectStat(self):
        ...
        self.assertSubSequence(
            (("Михаил", 3), ("Фёдор", 1), ("Павел", 5), ("Екатерина", 3), ("Ирина", 1)),
